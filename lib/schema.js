...
// note: > node 4 uses internal `assert`; < node 4 uses node-module
 ...
 */
function Schema (value, _parentPath) {
  var self = this
  ...
  value = clone(value)
  Schema.validate(value, _parentPath)
  ...
  function setProperties (value, children) {
    ...
    if (Array.isArray(value)) {
      ...
      value = value.map(function (item, i) {
        return new SubSchema(item, _parentPath + '[' + i + ']')
      })
    }
    // copy json properties
    assign(self, value)
    // all children queues and exchanges
    children = children || Schema._getChildren(self, _parentPath)
 ...
 */
Schema.validate = function (value, options, _parentPath) {
  _parentPath = _parentPath || ''
  if (Array.isArray(value)) {
    ...
    value.forEach(function (item, i) {
      jsonValidator(topologySchema, item, _parentPath + '[' + i + ']')
    })
  } else if (isObject(value)) {
    jsonValidator(topologySchema, value, _parentPath)
  } else {
    _parentPath = _parentPath || 'value'
    console.log('hai')
    throw new TypeError("'" + _parentPath + "' must be an object or array")
  }
}
/**
 ...
 */
Schema._getChildren = function (schema, _parentPath) {
  var children = {
    queues: [],
    exchanges: [],
    queuesByName: {},
  ...
  function indexChildren (schema) {
    if (isExchange(schema)) {
      schema.bindings.forEach(function (binding, i) {
        var dest = new SubSchema(binding.destination, _parentPath + '.bindings[' + i + ']')
...
Schema.prototype._exchangeValidateMessage = function (exchange, routingKey, message, _parentPath, _state) {
  _state = _state || { noDestinationQueuesFound: true }
